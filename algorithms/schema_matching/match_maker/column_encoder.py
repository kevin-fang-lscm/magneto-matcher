...
class ColumnEncoder:
    def __init__(self, tokenizer, encoding_mode="header_values_repeat", sampling_mode="mixed", n_samples=10):
        ...
        if encoding_mode not in self._serialization_methods:
            raise ValueError(f"Unsupported encoding mode: {encoding_mode}. Supported modes are: {
                             list(self._serialization_methods.keys())}")
        if sampling_mode not in sampling_modes:
            raise ValueError(f"Unsupported sampling mode: {
                             sampling_mode}. Supported modes are: {sampling_modes}")
    ...
    def encode(self, df, col):
        ...
        header = col
        tokens = get_samples(df[col], n=self.n_samples,
