...
class EmbeddingMatcher:
    def __init__(self, params):
        ...
        if self.model_name in DEFAULT_MODELS:
            ...
            if os.path.exists(model_path):
                ...
                print(f"Trained model not found at {
    ...
    def _get_embeddings_ft(self, texts, batch_size=32):
        ...
        for i in range(0, len(texts), batch_size):
            batch_texts = texts[i:i+batch_size]
            with torch.no_grad():
                batch_embeddings = self.model.encode(batch_texts, show_progress_bar=False, device=self.device)
    ...
    def get_embedding_similarity_candidates(self, source_df, target_df):
        encoder = ColumnEncoder(
            self.tokenizer, encoding_mode=self.params['encoding_mode'], sampling_mode=self.params['sampling_mode'], n_samples=self.params['sampling_size'])
