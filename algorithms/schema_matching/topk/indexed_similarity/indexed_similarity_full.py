...
class IndexedSimilarityMatcher(BaseMatcher):
    def __init__(self, filename, model_name='all-MiniLM-L6-v2', top_k: int = 40):
    ...
    def _insert_result(self, source_col, target_col, position):
        data_type = infer_column_type(self.source_df[source_col])
        self.results.append(("IndexedSimilarity", self.filename, "gdc_num_cat.csv", source_col, target_col, data_type, position))
    ...
    def _get_categorical_candidate_columns(self, column, clean_nulls=True, splitCamelCase=True):
        ...
        for value in values:
            results = self.catdomain_inverted_index.query(value, 40)
    ...
    def _handle_categorical_complex(self, ground_truth):
        ...
        for col in tqdm.tqdm(self.source_categorical_df.columns):
            ...
            if unique_ratio > 0.80:  # approximate key
                ...
                self._insert_result(col, "submitter_id", 0)
                # return cat_matches
                continue
            ...
            #     print('\n')
            self._insert_result(col, target, position)
    ...
    def get_matches(self, source_df: pd.DataFrame, target_df: pd.DataFrame, ground_truth) -> Dict[Tuple[Tuple[str, str], Tuple[str, str]], float]:
        ...
        cat_matches = self._handle_categorical_complex(ground_truth)

        num_matches = self._handle_numerical(ground_truth)

        binary_matches = self._handle_binary(ground_truth)
